...
import { useStyles } from "./HumidityCity.elements";
...
const HumidityCity = ({ id, data, inputs, outputs }) => {
	...
	return (
		<div className={classes.node}>
			...
			<div className={classes.body}>
				...
				>
					<TreeItem
						nodeId={`${treeHumidity.id}-${treeHumidity.city.id}`}
						label={treeHumidity.city.name}
					>
						{treeHumidity.city.children.map((item) => (
							<TreeItem
								nodeId={`${treeHumidity.id}-${treeHumidity.city.id}-${item.id}`}
								label={item.name}
								key={item.id}
							/>
						))}
