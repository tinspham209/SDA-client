...
import { useStyles } from "./RainfallYear.elements";
...
const RainfallYear = ({ id, data, inputs, outputs }) => {
	...
	return (
		<div className={classes.node}>
			...
			<div className={classes.body}>
				...
				>
					<TreeItem
						nodeId={`${treeRainfall.id}-${treeRainfall.year.id}`}
						label={treeRainfall.year.name}
					>
						{treeRainfall.year.children.map((item) => (
							<TreeItem
								nodeId={`${treeRainfall.id}-${treeRainfall.year.id}-${item.id}`}
								label={item.name}
								key={item.id}
							/>
						))}
