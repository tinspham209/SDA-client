...
import { useStyles } from "./TemperatureCity.elements";
...
const TemperatureCity = ({ id, data, inputs, outputs }) => {
	...
	return (
		<div className={classes.node}>
			...
			<div className={classes.body}>
				...
				>
					<TreeItem
						nodeId={`${treeTemperature.id}-${treeTemperature.city.id}`}
						label={treeTemperature.city.name}
					>
						{treeTemperature.city.children.map((item) => (
							<TreeItem
								nodeId={`${treeTemperature.id}-${treeTemperature.city.id}-${item.id}`}
								label={item.name}
								key={item.id}
							/>
						))}
